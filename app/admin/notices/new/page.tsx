...
export default function AdminNoticeEditPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        console.error("[admin notice edit load error]", err);
        if (!alive) return;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      console.error("[admin notice edit error]", err);
  ...
  return (
    <main className="min-h-screen bg-white px-4 py-6">
      <div className="mx-auto max-w-4xl">
        <div className="mb-4">
          <Link href={`/admin/notices/${noticeId}`} className="text-sm text-gray-500 hover:text-gray-800">
            ← 공지 상세
          </Link>
        </div>

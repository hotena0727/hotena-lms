...
export default function AdminCourseNewPage() {
  ...
  async function loadSelectableCourses() {
    try {
      ...
      const rows = ((data ?? []) as SelectableCourse[]).filter(
        (course) => (course.catalog_type ?? "single") === "single"
      );

      setAvailableCourses(rows);
    } catch (err: any) {
      setMessageType("error");
      setMessage(err?.message || "단과 강의 목록을 불러오지 못했습니다.");
    } finally {
  ...
  function removePackageItem(courseId: string) {
    setSelectedItems((prev) => prev.filter((item) => item.id !== courseId));
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!createdCourse) throw new Error("생성된 강의 정보를 찾을 수 없습니다.");

      createdCourseId = createdCourse.id;

      if (form.courseType === "package" && selectedItems.length > 0) {
        const packageRows = selectedItems.map((item, index) => ({
          package_course_id: createdCourse.id,
      ...
      if (createdCourseId) {
        await supabase
          .from("course_package_items")
          .delete()
          .eq("package_course_id", createdCourseId);
        await supabase.from("courses").delete().eq("id", createdCourseId);
      }

      setMessageType("error");
      setMessage(err?.message || "강의 등록 중 오류가 발생했습니다.");
    } finally {
      setSaving(false);
    }
  }
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-bold text-slate-900">새 강의 등록</h1>
            <p className="mt-4 max-w-3xl text-sm leading-8 text-slate-600 md:text-base">
              단과 강의는 직접 등록하고, 패키지는 기존 단과 강의를 선택해서 묶는 방식으로 구성합니다.
            </p>
          </div>
              ...
              >
                ...
                <p
                  className={`mt-2 text-sm leading-6 ${
                    form.courseType === item.key ? "text-white/85" : "text-slate-600"
                  }`}
                >
                  {item.desc}
                </p>

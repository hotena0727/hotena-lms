...
export default function AdminLessonNewPage() {
  ...
  useEffect(() => {
    ...
    async function loadCourses() {
      try {
        ...
        const rows = (data ?? []) as SelectableCourse[];
        setCourses(rows);

        if (rows.length > 0) {
          setForm((prev) => ({
            ...prev,
            course_id: prev.course_id || rows[0].id,
          }));
        }
      } catch (err: any) {
        if (!alive) return;
        setMessageType("error");
        setMessage(err?.message || "강의 목록을 불러오지 못했습니다.");
      } finally {
        if (!alive) return;
        setLoading(false);
      }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setMessageType("error");
      setMessage(err?.message || "레슨 등록 중 오류가 발생했습니다.");
    } finally {
      setSaving(false);
    }
  }
    ...
    return (
      <div className="space-y-6">
        <section className="rounded-3xl border border-slate-200 bg-white p-8">
          <p className="text-sm text-slate-500">레슨 등록 화면을 준비하는 중입니다.</p>
        </section>
      </div>
    );
  }
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-bold text-slate-900">새 레슨 등록</h1>
            <p className="mt-4 max-w-3xl text-sm leading-8 text-slate-600 md:text-base">
              강의를 선택한 뒤 레슨 제목, 영상 정보, 첨부 URL 등을 입력해서 수동으로 레슨을 등록할 수 있습니다.
            </p>
          </div>
